...
final class MovimentacaoServiceImpl implements MovimentacaoService {
  late final MovimentacaoRepository _movimentacaoRepository = ddi();

  @override
  Stream<MovimentacaoDados> buscarDadosDetalhamento() {
    return _movimentacaoRepository.filter().map((Map<String, List<MovimentacaoModel>> event) {
  ...
  Stream<Map<String, List<MovimentacaoModel>>> buscarDadosMovimentacao() {
    return _movimentacaoRepository.filter();
  }

  @override
  bool remover(int id) {
    return _movimentacaoRepository.remover(id);
  }

  @override
  int salvar(MovimentacaoEntity movimentacaoEntity) {
    return _movimentacaoRepository.salvar(movimentacaoEntity);
  }
}
